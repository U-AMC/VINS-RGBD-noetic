	...
	if ((int)matched_2d_cur.size() > MIN_LOOP_NUM)
	{
		...
		ROS_WARN("points left after RANSAC: %d", (int)matched_2d_cur.size());
	    	...
	    	if (DEBUG_IMAGE)
	        {
	            ...
	            putText(notation, "current frame: " + to_string(index) + "  sequence: " + to_string(sequence), cv::Point2f(20, 30), cv::FONT_HERSHEY_SCRIPT_SIMPLEX, 1, cv::Scalar(255), 3);

	            putText(notation, "previous frame: " + to_string(old_kf->index) + "  sequence: " + to_string(old_kf->sequence), cv::Point2f(20 + COL + gap, 30), cv::FONT_HERSHEY_SCRIPT_SIMPLEX, 1, cv::Scalar(255), 3);
